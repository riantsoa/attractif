...
foreach ($sale as $key=>$value)
{
    ...
    {
        echo form_datetime('date', $one_sale[0]->$key, 'class="form-control input-sm" required="required"  id="evenement_date"');
    }
    elseif ($sale[$key]["field"] == 'product')
    {
        echo form_dropdown('product', $products, $one_sale[0]->$key, 'class="form-control input-sm" required="required" ');
    }
    elseif ($sale[$key]["field"] == 'user')
    {
        echo form_dropdown('user', $users, $one_sale[0]->$key, 'class="form-control input-sm" required="required" ');
    }
    elseif ($sale[$key]["field"] == 'event')
    {
        echo form_dropdown('event', $events, $one_sale[0]->$key, 'class="form-control input-sm" required="required" ');
    }
    else
    {
        echo form_input($sale[$key]["field"], $one_sale[0]->$key, 'class="form-control input-sm" required="required" ');
